...
class TestTransformationProcessTemplate(Base):
    ...
    template_config = Column(JSON, nullable=False)  # Using JSON instead of JSONB
...
class TestNotificationTemplate(Base):
    ...
    variables = Column(JSON, nullable=True)  # Using JSON instead of JSONB
    is_active = Column(Boolean, default=True)
    created_at = Column(DateTime, default=func.now())
    updated_at = Column(DateTime, default=func.now(), onupdate=func.now())
...
class TestWebhookEndpoint(Base):
    ...
    events = Column(JSON, nullable=False)  # Using JSON instead of JSONB
