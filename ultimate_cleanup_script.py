#!/usr/bin/env python3
"""
Ultimate cleanup script that handles all remaining foreign key constraints.
"""
...
def main():
    print("🚀 Ultimate cleanup - handling all remaining foreign key constraints...")
    ...
    try:
        ...
        print(f"\n🗑️  Deleting {len(companies_to_delete)} remaining generic companies...")
        
        # Delete companies one by one with proper error handling
        deleted_count = 0
        for company_id, name, email in companies_to_delete:
            try:
                # Handle all foreign key constraints in the correct order
                ...
                # 3. Delete batch transactions (including those created by users from this company)
                cursor.execute("DELETE FROM batch_transactions WHERE created_by_user_id IN (SELECT id FROM users WHERE company_id = %s)", (company_id,))
                ...
                # 4. Handle circular dependency between batches and purchase orders
                # First, set batch_id to NULL in purchase_orders
                cursor.execute("UPDATE purchase_orders SET batch_id = NULL WHERE batch_id IN (SELECT id FROM batches WHERE company_id = %s)", (company_id,))
                
                # 5. Set source_purchase_order_id to NULL in batches
                cursor.execute("UPDATE batches SET source_purchase_order_id = NULL WHERE source_purchase_order_id IN (SELECT id FROM purchase_orders WHERE buyer_company_id = %s OR seller_company_id = %s)", (company_id, company_id))
                ...
                # 8. Delete business relationships
