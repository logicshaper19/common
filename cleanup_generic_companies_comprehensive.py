#!/usr/bin/env python3
"""
Comprehensively clean up generic companies from the database, handling all foreign key constraints.
"""
...
def cleanup_generic_companies_comprehensive():
    """Comprehensively remove generic companies from the database."""
    print("🧹 Comprehensively cleaning up generic companies from the database...")
    ...
    try:
        ...
        for company in generic_companies:
            ...
            # First, delete all batches associated with purchase orders from this company
            ...
            for po in pos_as_buyer + pos_as_seller:
                # Delete batches that reference this PO
                try:
                    result = db.execute(text("DELETE FROM batches WHERE source_purchase_order_id = :po_id"), {"po_id": po.id})
                    deleted_batches += result.rowcount
                    if result.rowcount > 0:
                        print(f"   📦 Deleted {result.rowcount} batches for PO: {po.po_number}")
                except Exception as e:
                    print(f"   ⚠️  Error deleting batches for PO {po.po_number}: {str(e)}")
        ...
        print(f"   📦 Deleted {deleted_batches} associated batches")
...
if __name__ == "__main__":
    success = cleanup_generic_companies_comprehensive()
    if success:
